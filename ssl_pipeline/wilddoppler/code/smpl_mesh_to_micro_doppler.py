...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mesh-cache-dir",
        default="",
        help="Optional directory for cached native SMPL-X meshes before radar placement.",
    )
    ...
    parser.add_argument(
        "--skip-plot",
        action="store_true",
        help="Skip PNG rendering and save only numeric outputs.",
    )
    parser.add_argument(
        "--max-radar-frames",
        type=int,
...
def bootstrap_witwin_modules():
    ...
    patch_slang_cuda_arch()

    return Radar, RadarConfig, Scene, Tracer


def slang_arch_to_gencode(value: str) -> list[str]:
    """Translate one Torch-style CUDA arch token into nvcc gencode flags."""
    token = value.replace(",", " ").replace(";", " ").split()[0].strip()
    want_ptx = token.upper().endswith("+PTX")
    arch = token.replace("+PTX", "").replace("+ptx", "").strip()
    if not arch:
        return []
    digits = arch.replace(".", "")
    if not digits.isdigit():
        return []
    if want_ptx:
        return [f"-gencode=arch=compute_{digits},code=compute_{digits}"]
    return [f"-gencode=arch=compute_{digits},code=sm_{digits}"]


def patch_slang_cuda_arch() -> None:
    arch = os.environ.get("WITWIN_SLANG_CUDA_ARCH") or os.environ.get("TORCH_CUDA_ARCH_LIST")
    if not arch:
        return
    extra_cuda_flags = slang_arch_to_gencode(arch)
    if not extra_cuda_flags:
        return

    try:
        import slangtorch
        from witwin.radar.solvers import solver_slang
    except Exception:
        return

    if getattr(solver_slang, "_wilddoppler_arch_patch", None) == tuple(extra_cuda_flags):
        return

    solver_slang._wilddoppler_arch_patch = tuple(extra_cuda_flags)

    def init_with_arch():
        solver_slang.ensure_current_env_on_path()
        slang_path = os.path.join(os.path.dirname(solver_slang.__file__), "radar.slang")
        return slangtorch.loadModule(slang_path, extraCudaFlags=extra_cuda_flags)

    solver_slang.init = init_with_arch
...
def mesh_cache_path(amass_path: Path, model_dir: Path, cache_dir: Path) -> Path:
    stat = amass_path.stat()
    key = "|".join(
        (
            str(amass_path.resolve()),
    ...
    return cache_dir / f"{amass_path.stem}_{digest}_smplx_mesh.npz"
...
class FrameLevelScattererInterpolator:
    ...
    def vertices_at_many(self, times_s: torch.Tensor) -> torch.Tensor:
        times_s = times_s.to(device=self.native_vertices.device, dtype=torch.float32)
        position = torch.clamp(times_s, 0.0, self.duration_s) * self.mocap_frame_rate
        low = torch.floor(position).to(dtype=torch.long).clamp_(0, self.last_native)
        high = (low + 1).clamp_(0, self.last_native)
        alpha = (position - low.to(dtype=position.dtype)).view(-1, 1, 1)
        return torch.lerp(self.native_vertices[low], self.native_vertices[high], alpha).contiguous()
    ...
    def collect_chirp_samples(self, radar, t0: float = 0.0):
        class BatchedTraceSamples(list):
            pass
        ...
        chirp_times = (
            float(t0)
            + torch.arange(cfg.chirp_per_frame, device=self.native_vertices.device, dtype=torch.float32)
            * (chirp_period * cfg.num_tx)
        )
        vertices = self.vertices_at_many(chirp_times)
        triangles = vertices[:, self.faces[self.visible_ids]]
        ...
        intensities = triangle_area * gaussian_specular_weight(
            flat_points,
            flat_normals,
            self.tx_position,
            self.rx_position,
        ).view(cfg.chirp_per_frame, -1)

        if self.visibility_start is not None and self.visibility_end is not None:
            duration = max(self.visibility_end_time_s - self.visibility_start_time_s, 1e-12)
            alpha = torch.clamp(
                (chirp_times - self.visibility_start_time_s) / duration,
                min=0.0,
                max=1.0,
            ).view(-1, 1)
            visibility = torch.lerp(self.visibility_start.view(1, -1), self.visibility_end.view(1, -1), alpha)
            intensities = intensities * visibility

        self.chirp_calls += int(cfg.chirp_per_frame)
        fast_slang = (
            str(getattr(radar, "backend", "")) == "slang"
            and os.environ.get("WITWIN_SLANG_FRAME_FLOAT32", "0").strip().lower() not in {"0", "false", "no"}
            and os.environ.get("WITWIN_SLANG_BATCH_PREP", "1").strip().lower() not in {"0", "false", "no"}
            and os.environ.get("WITWIN_SLANG_FRAME_PHASE_COEFF", "0").strip().lower() in {"0", "false", "no"}
            and os.environ.get("WITWIN_SLANG_FRAME_FUSED", "0").strip().lower() in {"0", "false", "no"}
            and os.environ.get("WITWIN_SLANG_FRAME_CHUNKED", "0").strip().lower() in {"0", "false", "no"}
            and getattr(radar, "polarization", None) is None
        )
        if fast_slang:
            samples = BatchedTraceSamples()
        else:
            samples = BatchedTraceSamples(
                [
                    self.trace_result_type(points[i], intensities[i], self.visible_ids, normals=normals[i])
                    for i in range(cfg.chirp_per_frame)
                ]
            )
        samples.points_batched = points.contiguous()
        samples.intensities_batched = intensities.contiguous()
        samples.entry_points_batched = samples.points_batched
        samples.fixed_path_lengths_batched = torch.zeros_like(intensities)
        samples.normals_batched = normals.contiguous()
        return samples
...
def main() -> int:
    ...
    image_paths = () if args.skip_plot else (image_path,)
    maybe_doppler_paths = (doppler_time_path,) if args.save_frame_doppler_time else ()
    maybe_doppler_image_paths = (
        (doppler_time_image_path,) if args.save_frame_doppler_time and not args.skip_plot else ()
    )
    existing = [
        path
        for path in (result_path, *image_paths, summary_path, *maybe_doppler_paths, *maybe_doppler_image_paths)
        if path.exists()
    ]
    ...
    if args.mesh_cache_dir:
        ...
        mesh_cache = mesh_cache_path(amass_path, model_dir, mesh_cache_dir)
    ...
    summary = {
        ...
        "image_png": None if args.skip_plot else str(image_path),
        "doppler_time_npz": str(doppler_time_path) if frame_doppler is not None else None,
        "doppler_time_png": str(doppler_time_image_path) if frame_doppler is not None and not args.skip_plot else None,
    }
