...
PRETRAIN_CKPT="${PRETRAIN_CKPT:-self_supervised_pretrain/checkpoints/contrastive_a2_amass_equal/contrastive_mobilenet_v2_last.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/a2_contrastive_downstream_full}"
TASKS="${TASKS:-MotionState VelocityRegression SingleHand}"
DEVICE_LIST="${DEVICE_LIST:-cuda:0,cuda:1,cuda:2}"
MAX_JOBS="${MAX_JOBS:-3}"
...
run_one() {
  ...
  echo "== A2 downstream full: ${task} ${tag} on ${device} =="
