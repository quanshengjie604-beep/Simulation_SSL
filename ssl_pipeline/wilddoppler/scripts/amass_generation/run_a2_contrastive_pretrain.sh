...
CKPT_DIR="${CKPT_DIR:-self_supervised_pretrain/checkpoints/contrastive_a2_amass_equal}"
TRACKLIST_CSV="${TRACKLIST_CSV:-data/fold_splits/AMASS_equal_target_tracklist.csv}"
DATA_DIR="${DATA_DIR:-data/amass_equal_target_tracks_Doppler}"

export PYTHONPATH="$(dirname "${ROOT}"):${ROOT}:${PYTHONPATH:-}"

GPU_LIST_ARGS=()
if [[ -n "${GPU_LIST}" ]]; then
  GPU_LIST_ARGS=(--gpu-list "${GPU_LIST}")
fi
