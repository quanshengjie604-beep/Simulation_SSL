...
class MAELinearProbeClassifier(nn.Module):
    def __init__(
        self,
        model: nn.Module,
        embed_dim: int,
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if embedding is None:
            raise KeyError("MAE embedding wrapper outputs must include 'embedding' or 'pooled'.")
...
def _run_supervised_linear_probe(
    ...
    model = MAELinearProbeClassifier(
        copy.deepcopy(mae_model),
        embed_dim=embed_dim,
...
def _run_mae_tune_for_ckpt(
    ...
    if is_regression:
        print(
            "Regression task detected; skipping viz, KNN probes, and frozen-backbone heads. "
            "Only full-finetune linear-probe variants will run."
        )
    ...
    cls_cfg_template.paths.encoder_type = "reconstruction"
    ...
    mae_model, mae_cfg, ckpt = load_mae_checkpoint(
        ckpt_path,
        device,
        config_path=cfg_path_override,
    )
    embed_wrapper = MAEEmbeddingWrapper(mae_model, embedding_key=embedding_key).to(device)
    embed_wrapper.eval()

    embed_dim = resolve_mae_embedding_dim(mae_model)

    epoch = ckpt.get("epoch", "unknown")
    print(f"MAE checkpoint epoch: {epoch}")
    ...
    for _fold_idx in range(_n_iters):
        ...
        for split_name, loader in eval_loaders.items():
            print(f"Extracting MAE embeddings for '{split_name}' split...")
...
def main(cfg: DictConfig):
    ...
    cfg.method_name = "reconstruction"
    ...
    if not ckpt_specs:
        raise RuntimeError("No MAE checkpoints configured under 'paths'.")
