...
def save_pred(pred, root, checkpoint_name, dataset_split, seq_id_to_name=None):
    seq_id_to_name = seq_id_to_name or {}
...
def main():
    ...
    if getattr(cfg, 'disable_cudnn', False):
        torch.backends.cudnn.enabled = False
    torch.backends.cudnn.benchmark = getattr(cfg, 'cudnn_benchmark', torch.backends.cudnn.benchmark)
    ...
    if not distributed and "CUDA_VISIBLE_DEVICES" not in os.environ:
    ...
    test_batch_size = args.batch_size or getattr(cfg, 'test_samples_per_gpu', cfg.data.samples_per_gpu)
    test_workers = args.workers or getattr(cfg, 'test_workers_per_gpu', cfg.data.workers_per_gpu)
    if args.speed_test:
        test_batch_size = 1
    enable_amp = getattr(cfg, 'enable_amp', False) if args.amp is None else args.amp
    ...
    save_pred(predictions, args.work_dir, checkpoint_file_name,
              'test' if args.testset else 'train', dataset.seq_id_to_name)

    result_dict, _ = dataset.evaluation(predictions, output_dir=args.work_dir, testset=args.testset)
