...
class CenterHead(nn.Module):
    ...
    def _get_grid(self, H, W, L, device, dtype):
        key = (H, W, L, str(device), str(dtype))
        ...
        if cached is None:
            ...
            cached = (
                zs.reshape(1, -1, 1).to(dtype=dtype),
                ys.reshape(1, -1, 1).to(dtype=dtype),
                xs.reshape(1, -1, 1).to(dtype=dtype),
            )
    ...
    def predict(self, example, preds_dicts, test_cfg, app_emb_tasks=None, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_pts_preds = []
            zs, ys, xs = self._get_grid(H, W, L, batch_hm.device, batch_hm.dtype)
            zs = zs.expand(batch, -1, -1)
            ys = ys.expand(batch, -1, -1)
            xs = xs.expand(batch, -1, -1)
