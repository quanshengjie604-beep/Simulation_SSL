...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.write_iter_loss_csv = write_iter_loss_csv
        self.scaler = torch.cuda.amp.GradScaler(enabled=enable_amp)
        self.epoch_eval_loaders = []
        self.epoch_eval_names = []
        self.epoch_eval_testsets = []
        self.epoch_eval_interval = 0
        self.epoch_eval_intervals = []
        self._iter_loss_csv_fields = None
    ...
    def set_epoch_evaluators(self, loaders, names, testsets, interval=1, intervals=None):
        self.epoch_eval_loaders = loaders
        self.epoch_eval_names = names
        self.epoch_eval_testsets = testsets
        self.epoch_eval_interval = interval
        if intervals is None:
            self.epoch_eval_intervals = [interval for _ in loaders]
        else:
            assert len(intervals) == len(loaders)
            self.epoch_eval_intervals = intervals

    def _write_iter_loss(self, epoch, inner_iter, log_vars):
        if not self.write_iter_loss_csv or self.rank != 0 or self.work_dir is None:
            return
        row = OrderedDict()
        row["epoch"] = epoch + 1
        row["iter"] = inner_iter + 1
        row["global_iter"] = self.iter + 1
        row.update(log_vars)
        path = osp.join(self.work_dir, "iter_losses.csv")
        if self._iter_loss_csv_fields is None:
            self._iter_loss_csv_fields = list(row.keys())
        write_header = not osp.exists(path)
        with open(path, "a", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=self._iter_loss_csv_fields)
            if write_header:
                writer.writeheader()
            writer.writerow(row)

    def _write_epoch_train_summary(self, epoch, log_var_sums, num_iters):
        if self.rank != 0 or self.work_dir is None or num_iters == 0:
            return
        row = OrderedDict()
        row["epoch"] = epoch + 1
        row["num_iters"] = num_iters
        for key in sorted(log_var_sums.keys()):
            row[key] = log_var_sums[key] / num_iters

        path = osp.join(self.work_dir, "epoch_train_summary.csv")
        write_header = not osp.exists(path)
        with open(path, "a", newline="") as f:
    ...
    def _append_eval_group_summary(self, filename, epoch, split_name, group_key, group_results):
        if self.rank != 0 or self.work_dir is None or not group_results:
            return
        metric_keys = sorted(
            {
                key
                for metrics in group_results.values()
                for key, value in metrics.items()
                if isinstance(value, (int, float, np.integer, np.floating))
            }
        )
        path = osp.join(self.work_dir, filename)
        fieldnames = ["epoch", "split", group_key] + metric_keys
        write_header = not osp.exists(path)
        with open(path, "a", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            if write_header:
                writer.writeheader()
            for group_name, metrics in sorted(group_results.items()):
                row = OrderedDict()
                row["epoch"] = epoch
    ...
    def evaluate_pose_loader(self, data_loader, split_name, epoch, testset=False, **kwargs):
        self.model.eval()
        detections = {}
        ...
        for data_batch in data_loader:
            ...
            for output in outputs:
                ...
                detections[f"{seq_name}/{frame}/{rdr_frame_name}"] = output
        ...
        eval_dir = osp.join(self.work_dir, f"epoch_{epoch:03d}", split_name)
        torchie.mkdir_or_exist(eval_dir)
        result_dict, _ = data_loader.dataset.evaluation(
            predictions, output_dir=eval_dir, testset=testset
        )

        checkpoint_name = f"epoch_{epoch:03d}"
        with open(osp.join(eval_dir, f"{checkpoint_name}_{split_name}_seq_results.json"), "w") as f:
            json.dump(dict(sorted(result_dict["seq_results"].items())), f, indent=2)
        with open(osp.join(eval_dir, f"{checkpoint_name}_{split_name}_results.json"), "w") as f:
            json.dump(result_dict, f, indent=2)

        results = result_dict["results"]
        self.logger.info("Epoch %d %s evaluation", epoch, split_name)
        for key, value in results.items():
            self.logger.info("Evaluation %s/%s: %s", split_name, key, value)

        summary_path = osp.join(self.work_dir, "epoch_eval_summary.csv")
        row = OrderedDict()
        row["epoch"] = epoch
        row["split"] = split_name
        ...
        with open(summary_path, "a", newline="") as f:
            ...
            if write_header:
                writer.writeheader()
            writer.writerow(row)
        self._append_eval_group_summary(
            "epoch_motion_summary.csv",
            epoch,
            split_name,
            "motion",
            result_dict.get("motion_results", {}),
        )
        self._append_eval_group_summary(
            "epoch_joint_summary.csv",
            epoch,
            split_name,
            "joint",
            result_dict.get("joint_results", {}),
        )
    ...
    def train(self, data_loader, epoch, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            if "log_vars" in outputs:
                ...
                self._write_iter_loss(epoch, i, outputs["log_vars"])
                for key, value in outputs["log_vars"].items():
                    if isinstance(value, (int, float, np.integer, np.floating)):
                        log_var_sums[key] = log_var_sums.get(key, 0.0) + float(value)
                num_train_iters += 1
        ...
        self._write_epoch_train_summary(epoch, log_var_sums, num_train_iters)
        self._epoch += 1
        if self.epoch_eval_intervals:
            for eval_loader, eval_name, eval_testset, eval_interval in zip(
                self.epoch_eval_loaders,
                self.epoch_eval_names,
                self.epoch_eval_testsets,
                self.epoch_eval_intervals,
            ):
                if eval_interval and self.epoch % eval_interval == 0:
                    self.evaluate_pose_loader(
                        eval_loader,
                        eval_name,
                        self.epoch,
                        testset=eval_testset,
                        **kwargs,
                    )
