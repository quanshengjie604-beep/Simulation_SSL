...
from det3d.datasets import DATASETS, build_dataloader, build_dataset
...
def batch_processor(model, data, train_mode, **kwargs):
    ...
    if train_mode:
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=kwargs.get("enable_amp", False)):
            losses = model(example, return_loss=True)
        loss, log_vars = parse_second_losses(losses)
        num_samples = len(example.get("meta", []))
        if num_samples == 0:
            first_sensor = next(iter(example.values()))
            num_samples = next(iter(first_sensor.values())).shape[0]
...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    data_loaders = [
        build_dataloader(
            ...
            dist=distributed,
            prefetch_factor=getattr(cfg.data, 'prefetch_factor', 1),
        )
        for ds in dataset
    ]
    epoch_eval_interval = getattr(cfg, 'eval_epoch_interval', 0)
    epoch_eval_loaders = []
    epoch_eval_names = []
    epoch_eval_testsets = []
    epoch_eval_intervals = []
    val_eval_interval = getattr(cfg, 'val_eval_epoch_interval', epoch_eval_interval)
    test_eval_interval = getattr(cfg, 'test_eval_epoch_interval', epoch_eval_interval)
    if val_eval_interval or test_eval_interval:
        test_batch_size = getattr(cfg, 'test_samples_per_gpu', cfg.data.samples_per_gpu)
        test_workers = getattr(cfg, 'test_workers_per_gpu', cfg.data.workers_per_gpu)
        for split_name, split_cfg, is_testset, split_interval in [
            ('val', cfg.data.val, False, val_eval_interval),
            ('test', cfg.data.test, True, test_eval_interval),
        ]:
            if not split_interval:
                continue
            eval_dataset = build_dataset(split_cfg)
            epoch_eval_loaders.append(
                build_dataloader(
                    eval_dataset,
                    batch_size=test_batch_size,
                    workers_per_gpu=test_workers,
                    dist=distributed,
                    shuffle=False,
                    prefetch_factor=getattr(cfg.data, 'prefetch_factor', 1),
                )
            )
            epoch_eval_names.append(split_name)
            epoch_eval_testsets.append(is_testset)
            epoch_eval_intervals.append(split_interval)
    ...
    trainer = Trainer(
        ...
        write_iter_loss_csv=getattr(cfg, "write_iter_loss_csv", False),
    )
    if epoch_eval_intervals:
        trainer.set_epoch_evaluators(
            epoch_eval_loaders,
            epoch_eval_names,
            epoch_eval_testsets,
            interval=epoch_eval_interval,
            intervals=epoch_eval_intervals,
        )
