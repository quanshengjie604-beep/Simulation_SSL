...
class PreparePoseInput(object):
    def __call__(self, res, info):
        res_new = {'meta': res['meta']}
        if 'rdr_cube' in res:
            rdr_tensor = res['rdr_cube']
            if len(rdr_tensor.shape) < 4:
                rdr_tensor = np.expand_dims(rdr_tensor, axis=0)
            elif len(rdr_tensor.shape) > 4:
                rdr_tensor = rdr_tensor.reshape(-1, *list(rdr_tensor.shape)[2:])
            res_new['rdr'] = {'rdr_tensor': rdr_tensor}
        return res_new, info
